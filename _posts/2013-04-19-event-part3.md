...
jQuery自己写了一个基于native event的`Event`对象，并且把`copy`数组中对应的属性从native event中复制到自己的`Event`对象中。然后，jQuery纠正了`event.target`对象：
...
这个方法有点复杂，暂时不分析，但其作用是在所有`handlers`中筛选出所有需要处理的`handler`。（可能你会想，绑定在elem中，对应`event.type`的`handler`不是全部都要处理的吗？仔细想？对了，就是代理，哈哈）。
